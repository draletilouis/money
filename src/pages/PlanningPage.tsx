...
import { CalendarClock, CircleAlert, Flag, Plus, ReceiptText, TrendingDown, TrendingUp, WalletCards } from 'lucide-react';
...
export function PlanningPage() {
  ...
  return <div className="page">
    ...
    {loading ? <Loading label="Loading your plan"/> : error ? <ErrorState message={error} retry={reload}/> : data && <>
      {tab === 'budgets' && <Budgets budgets={data.budgets} onCreate={() => setForm('budgets')}/>}
...
function Budgets({ budgets, onCreate }: { budgets: Budget[]; onCreate: () => void }) {
...
function BudgetForm({ profileId, categories, onClose, onCreated }: FormProps & { categories: Category[] }) {
  const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const expenses = categories.filter((item) => item.type === 'EXPENSE');
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); setError(''); const data = new FormData(event.currentTarget); try { await api(`/profiles/${profileId}/budgets`, { method: 'POST', body: JSON.stringify({ name: data.get('name'), categoryId: data.get('categoryId'), amount: Number(data.get('amount')), startDate: data.get('startDate'), endDate: data.get('endDate'), alertThreshold: Number(data.get('alertThreshold')) }) }); onCreated(); } catch (caught) { setError(messageFrom(caught, 'Could not create this budget.')); } finally { setBusy(false); } };
  return <Modal title="Create a budget" description="Spending is calculated from posted Money Out transactions." onClose={onClose}><form className="form-stack" onSubmit={submit}>{error && <div className="form-error">{error}</div>}<label><span>Budget name</span><input name="name" placeholder="e.g. Fuel budget" required autoFocus/></label><div className="form-grid"><SelectField label="Expense category" name="categoryId" options={expenses.map((item) => [item.id, item.name])}/><MoneyField label="Amount" name="amount"/><DateField label="Starts" name="startDate" value={new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().slice(0, 10)}/><DateField label="Ends" name="endDate" value={monthEnd()}/><label><span>Warn me at</span><select name="alertThreshold" defaultValue="80"><option value="70">70%</option><option value="80">80%</option><option value="90">90%</option><option value="100">100%</option></select></label></div><FormActions busy={busy} submit="Create budget" onClose={onClose}/></form></Modal>;
}
