import { useCallback, useEffect, useState, type ChangeEventHandler, type FormEvent, type ReactNode } from 'react';
...
type GoalMovement = { id: string; type: 'OPENING_BALANCE' | 'CONTRIBUTION' | 'WITHDRAWAL'; amount: string; movementDate: string; notes?: string; account?: Account };
type Goal = { id: string; name: string; goalType: string; targetAmount: string; currentAmount: string; remainingAmount: string; requiredMonthlyContribution?: string | null; monthsRemaining?: number | null; scheduleStatus: string; targetDate?: string; status: string; linkedAccountId?: string; linkedAccount?: Account; movements?: GoalMovement[] };
...
const monthStart = () => { const now = new Date(); return dateInput(new Date(now.getFullYear(), now.getMonth(), 1)); };
const monthEnd = () => { const now = new Date(); return dateInput(new Date(now.getFullYear(), now.getMonth() + 1, 0)); };
const messageFrom = (caught: unknown, fallback: string) => caught instanceof ApiError ? caught.message : fallback;
const matchingBudget = (budgets: Budget[], categoryId: string, date: string) => budgets.find((budget) => budget.category.id === categoryId && date >= budget.startDate.slice(0, 10) && date <= budget.endDate.slice(0, 10));
const goalAccounts = (accounts: Account[]) => accounts.filter((account) => !['CREDIT_CARD', 'LOAN'].includes(account.type));
...
export function PlanningPage() {
  ...
  const [form, setForm] = useState<FormKind | null>(null); const [settlement, setSettlement] = useState<Settlement | null>(null); const [goalDetails, setGoalDetails] = useState<Goal | null>(null);
  ...
  const archiveBudget = async (budget: Budget) => {
    if (!window.confirm(`Archive “${budget.category.name} budget”? Its history will be preserved.`)) return;
  ...
  return <div className="page">
    ...
    {loading ? <Loading label="Loading your plan"/> : error ? <ErrorState message={error} retry={reload}/> : data && <>
      ...
      {tab === 'goals' && (
        <PlanningList empty="No goals have been created." emptyAction={<button className="button primary" onClick={() => setForm('goals')}><Plus/> Create goal</button>} items={data.goals.map((item) => ({ id: item.id, icon: <Flag/>, title: item.name, subtitle: `${formatMoney(item.currentAmount)} of ${formatMoney(item.targetAmount)}${item.requiredMonthlyContribution && Number(item.requiredMonthlyContribution) > 0 ? ` · ${formatMoney(item.requiredMonthlyContribution)}/month needed` : ''}`, amount: `${Math.min(100, Math.round(Number(item.currentAmount) / Number(item.targetAmount) * 100))}%`, status: item.scheduleStatus, actions: <button className="button secondary tiny" onClick={() => setGoalDetails(item)}>Manage</button> }))}/>
      )}
    ...
    {form === 'bills' && (
      <BillForm profileId={selectedProfileId} categories={categories} accounts={accounts} budgets={data?.budgets ?? []} onClose={() => setForm(null)} onCreated={() => { setForm(null); reload(); }}/>
    )}
    ...
    {form === 'goals' && (
      <GoalForm profileId={selectedProfileId} accounts={goalAccounts(accounts)} onClose={() => setForm(null)} onCreated={() => { setForm(null); reload(); }}/>
    )}
    {settlement && (
      <SettlementForm profileId={selectedProfileId} settlement={settlement} accounts={accounts} budgets={data?.budgets ?? []} onClose={() => setSettlement(null)} onCompleted={() => { setSettlement(null); reload(); }}/>
    )}
    {goalDetails && (
      <GoalDetailsModal profileId={selectedProfileId} goal={goalDetails} accounts={goalAccounts(accounts)} onClose={() => setGoalDetails(null)} onChanged={reload}/>
    )}
  </div>;
}
...
function Budgets({ budgets, onCreate, onEdit, onArchive, onMovements }: { budgets: Budget[]; onCreate: () => void; onEdit: (budget: Budget) => void; onArchive: (budget: Budget) => void; onMovements: (budget: Budget) => void }) {
  return budgets.length ? <div className="budget-grid">{budgets.map((budget) => {
    ...
    return <article className={`card budget-card ${budget.thresholdReached ? 'budget-warning' : ''}`} key={budget.id}>
      <div><span className="category-dot"/><div><strong>{budget.category.name} budget</strong><small>{formatDate(budget.startDate)} – {formatDate(budget.endDate)}</small></div><b>{displayPercent}%</b></div>
...
function BudgetForm({ profileId, categories, budget, onClose, onCreated }: FormProps & { categories: Category[]; budget?: Budget }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); setError(''); const data = new FormData(event.currentTarget); try { await api(`/profiles/${profileId}/budgets${budget ? `/${budget.id}` : ''}`, { method: budget ? 'PUT' : 'POST', body: JSON.stringify({ categoryId: data.get('categoryId'), amount: Number(data.get('amount')), startDate: data.get('startDate'), endDate: data.get('endDate'), alertThreshold: Number(data.get('alertThreshold')) }) }); onCreated(); } catch (caught) { setError(messageFrom(caught, `Could not ${budget ? 'update' : 'create'} this budget.`)); } finally { setBusy(false); } };
  return <Modal title={budget ? 'Edit budget' : 'Create a budget'} description="Choose what you want to control. The category automatically becomes the budget name." onClose={onClose}><form className="form-stack" onSubmit={submit}>{error && <div className="form-error">{error}</div>}<SelectField label="Expense category" name="categoryId" options={expenses.map((item) => [item.id, item.name])} defaultValue={budget?.category.id}/><div className="form-grid"><MoneyField label="Budget amount" name="amount" defaultValue={budget?.amount}/><label><span>Warn me at</span><select name="alertThreshold" defaultValue={String(budget?.alertThreshold ?? 80)}><option value="70">70%</option><option value="80">80%</option><option value="90">90%</option><option value="100">100%</option></select></label><DateField label="Starts" name="startDate" value={budget?.startDate.slice(0, 10) ?? monthStart()}/><DateField label="Ends" name="endDate" value={budget?.endDate.slice(0, 10) ?? monthEnd()}/></div><FormActions busy={busy} submit={budget ? 'Save changes' : 'Create budget'} onClose={onClose}/></form></Modal>;
}
...
function BillForm({ profileId, categories, accounts, budgets, onClose, onCreated }: FormProps & { categories: Category[]; accounts: Account[]; budgets: Budget[] }) {
  ...
  return <Modal title="Add a bill" description="Track an upcoming payment and see which budget it will affect." onClose={onClose}><form className="form-stack" onSubmit={submit}>{error && <div className="form-error">{error}</div>}<label><span>Payee</span><input name="payee" placeholder="e.g. UMEME" required autoFocus/></label><div className="form-grid"><MoneyField label="Amount" name="amount"/><DateField label="Due date" name="dueDate" value={dueDate} onChange={(event) => setDueDate(event.target.value)}/><SelectField label="Expense category" name="categoryId" options={expenses.map((item) => [item.id, item.name])} onChange={(event) => setCategoryId(event.target.value)}/><SelectField label="Preferred payment account" name="paymentAccountId" options={accounts.map((item) => [item.id, item.name])} optional/></div><BudgetMatchNotice selected={Boolean(categoryId)} budget={budget} planning/><label><span>Notes</span><textarea name="notes" rows={3}/></label><FormActions busy={busy} submit="Add bill" onClose={onClose}/></form></Modal>;
}
...
function GoalForm({ profileId, accounts, onClose, onCreated }: FormProps & { accounts: Account[] }) {
  ...
  return <Modal title="Create a goal" description="Set a target and choose the account whose money will be earmarked for it." onClose={onClose}><form className="form-stack" onSubmit={submit}>{error && <div className="form-error">{error}</div>}{!accounts.length && <div className="form-error">Create a financial account before creating a goal.</div>}<label><span>Goal name</span><input name="name" placeholder="e.g. Emergency fund" required autoFocus/></label><div className="form-grid"><SelectField label="Goal type" name="goalType" options={[["SAVINGS", "Savings"], ["EMERGENCY_FUND", "Emergency fund"], ["PURCHASE", "Purchase"], ["DEBT_REPAYMENT", "Debt repayment"], ["INVESTMENT", "Investment"], ["OTHER", "Other"]]}/><MoneyField label="Target amount" name="targetAmount"/><MoneyField label="Already allocated" name="currentAmount" allowZero/><DateField label="Target date" name="targetDate" optional/><SelectField label="Holding account" name="linkedAccountId" options={accounts.map((item) => [item.id, item.name])}/></div><div className="integrity-note"><Flag/><div><strong>Goal allocations do not change the account balance</strong><p>They earmark part of the selected account so you can see how much is reserved for this goal.</p></div></div><label><span>Description</span><textarea name="description" rows={3}/></label><FormActions busy={busy || !accounts.length} submit="Create goal" onClose={onClose}/></form></Modal>;
}

function SettlementForm({ profileId, settlement, accounts, budgets, onClose, onCompleted }: { profileId: string; settlement: Settlement; accounts: Account[]; budgets: Budget[]; onClose: () => void; onCompleted: () => void }) {
  const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const [transactionDate, setTransactionDate] = useState(today()); const item = settlement.item; const bill = settlement.kind === 'bill'; const preferred = bill ? (item as Bill).paymentAccountId : (item as ExpectedIncome).destinationAccountId; const budget = bill ? matchingBudget(budgets, (item as Bill).category.id, transactionDate) : undefined;
  ...
  return <Modal title={bill ? 'Mark bill as paid' : 'Mark income as received'} description="This action posts a real transaction to the ledger and cannot be silently deleted." onClose={onClose}><form className="form-stack" onSubmit={submit}>{error && <div className="form-error">{error}</div>}<div className="settlement-summary"><strong>{bill ? (item as Bill).payee : (item as ExpectedIncome).source}</strong><span>{formatMoney(item.amount)}</span></div>{accounts.length ? <><SelectField label={bill ? 'Paid from' : 'Deposited into'} name="accountId" options={accounts.map((account) => [account.id, `${account.name} · ${formatMoney(account.balance)}`])} defaultValue={preferred}/><DateField label={bill ? 'Payment date' : 'Received date'} name="transactionDate" value={transactionDate} onChange={(event) => setTransactionDate(event.target.value)}/>{bill && <BudgetMatchNotice selected budget={budget}/>}</> : <div className="form-error">Create a financial account before settling this item.</div>}<FormActions busy={busy || !accounts.length} submit={bill ? 'Post payment' : 'Post income'} onClose={onClose}/></form></Modal>;
}

function BudgetMatchNotice({ selected, budget, planning = false }: { selected: boolean; budget?: Budget; planning?: boolean }) {
  if (!selected) return null;
  return budget ? <div className="budget-match-note"><ReceiptText/><div><strong>{planning ? 'Planned for' : 'Counts toward'}: {budget.category.name} budget</strong><small>{planning ? 'This assumes the bill is paid on the selected due date.' : 'Posting this payment will immediately increase the budget’s spent amount.'}</small></div></div> : <div className="budget-match-note none"><CircleAlert/><div><strong>No matching active budget</strong><small>The payment will still be recorded, but no budget covers this category and date.</small></div></div>;
}

function GoalDetailsModal({ profileId, goal, accounts, onClose, onChanged }: { profileId: string; goal: Goal; accounts: Account[]; onClose: () => void; onChanged: () => void }) {
  const [data, setData] = useState<Goal | null>(null); const [error, setError] = useState(''); const [action, setAction] = useState<'CONTRIBUTION' | 'WITHDRAWAL' | null>(null); const [busy, setBusy] = useState(false);
  const load = useCallback(() => { setError(''); api<Goal>(`/profiles/${profileId}/goals/${goal.id}`).then(setData).catch((caught) => setError(messageFrom(caught, 'Could not load this goal.'))); }, [profileId, goal.id]);
  useEffect(load, [load]);
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (!action) return; setBusy(true); setError(''); const form = new FormData(event.currentTarget);
    try { const updated = await api<Goal>(`/profiles/${profileId}/goals/${goal.id}/movements`, { method: 'POST', body: JSON.stringify({ type: action, amount: Number(form.get('amount')), accountId: form.get('accountId'), movementDate: form.get('movementDate'), notes: form.get('notes') || undefined }) }); setData(updated); setAction(null); onChanged(); }
    catch (caught) { setError(messageFrom(caught, `Could not record this ${action === 'CONTRIBUTION' ? 'contribution' : 'withdrawal'}.`)); }
    finally { setBusy(false); }
  };
  const current = Number(data?.currentAmount ?? goal.currentAmount); const target = Number(data?.targetAmount ?? goal.targetAmount); const percent = target > 0 ? Math.min(100, Math.round(current / target * 100)) : 0;
  return <Modal title={goal.name} description="Progress comes from dated contributions and withdrawals." onClose={onClose} wide>{error && <ErrorState message={error} retry={load}/>} {!data ? <Loading label="Loading goal"/> : <div className="goal-details"><div className="goal-progress-head"><div><span>Progress</span><strong>{formatMoney(data.currentAmount)} <small>of {formatMoney(data.targetAmount)}</small></strong></div><b>{percent}%</b></div><div className="progress"><i style={{ width: `${percent}%` }}/></div><div className="goal-summary"><span>Remaining<strong>{formatMoney(data.remainingAmount)}</strong></span><span>Monthly needed<strong>{data.requiredMonthlyContribution ? formatMoney(data.requiredMonthlyContribution) : 'No target date'}</strong></span><span>Schedule<strong>{data.scheduleStatus.replaceAll('_', ' ').toLowerCase()}</strong></span></div><div className="goal-actions"><button className="button primary" onClick={() => setAction('CONTRIBUTION')}><Plus/> Contribute</button><button className="button secondary" disabled={current <= 0} onClick={() => setAction('WITHDRAWAL')}><TrendingDown/> Withdraw</button></div><div className="integrity-note"><Flag/><div><strong>These movements earmark money</strong><p>Your financial account balance is unchanged; withdrawing releases money from the goal allocation.</p></div></div>{action && <form className="form-stack goal-movement-form" onSubmit={submit}><h3>{action === 'CONTRIBUTION' ? 'Add contribution' : 'Release goal money'}</h3><div className="form-grid"><MoneyField label="Amount" name="amount"/><SelectField label={action === 'CONTRIBUTION' ? 'Allocate from' : 'Release to'} name="accountId" options={accounts.map((account) => [account.id, account.name])} defaultValue={data.linkedAccount?.id}/><DateField label="Date" name="movementDate" value={today()}/></div><label><span>Notes</span><textarea name="notes" rows={2} placeholder="Optional explanation"/></label><FormActions busy={busy} submit={action === 'CONTRIBUTION' ? 'Save contribution' : 'Save withdrawal'} onClose={() => setAction(null)}/></form>}<div className="goal-history"><div className="section-heading"><div><p className="eyebrow">Audit trail</p><h2>Goal movements</h2></div></div>{data.movements?.length ? <div className="budget-movement-list">{data.movements.map((movement) => <div key={movement.id}><span className="activity-icon neutral">{movement.type === 'WITHDRAWAL' ? <TrendingDown/> : <TrendingUp/>}</span><div><strong>{movement.type === 'OPENING_BALANCE' ? 'Opening amount' : movement.type === 'CONTRIBUTION' ? 'Contribution' : 'Withdrawal'}</strong><small>{movement.account?.name ?? 'Unassigned account'} · {formatDate(movement.movementDate)}{movement.notes ? ` · ${movement.notes}` : ''}</small></div><b className={movement.type === 'WITHDRAWAL' ? 'negative' : ''}>{movement.type === 'WITHDRAWAL' ? '−' : '+'}{formatMoney(movement.amount)}</b></div>)}</div> : <EmptyState title="No movements yet" message="Contributions and withdrawals will appear here."/>}</div></div>}</Modal>;
}

function SelectField({ label, name, options, optional = false, defaultValue, onChange }: { label: string; name: string; options: string[][]; optional?: boolean; defaultValue?: string; onChange?: ChangeEventHandler<HTMLSelectElement> }) { return <label><span>{label}</span><select name={name} required={!optional} defaultValue={defaultValue ?? ''} onChange={onChange}><option value="" disabled={!optional}>{optional ? 'Not selected' : 'Select one'}</option>{options.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></label>; }
...
function DateField({ label, name, value, optional = false, onChange }: { label: string; name: string; value?: string; optional?: boolean; onChange?: ChangeEventHandler<HTMLInputElement> }) { return <label><span>{label}</span><input name={name} type="date" defaultValue={value} required={!optional} onChange={onChange}/></label>; }
