...
export const formatMoney = (amount: string | number, currency = 'UGX', compact = false) => currency === 'UGX'
  ? `UGX ${new Intl.NumberFormat('en-UG', { maximumFractionDigits: 0, notation: compact ? 'compact' : 'standard' }).format(Number(amount))}`
  : new Intl.NumberFormat('en-UG', { style: 'currency', currency, maximumFractionDigits: 2, notation: compact ? 'compact' : 'standard' }).format(Number(amount));
